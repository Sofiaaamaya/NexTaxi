    ...
    <div className="p-6 rounded-2xl shadow-md border bg-white relative">
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-gray-50/50 border-b border-gray-100">
              <th className="p-4"><Poppins text={t('dashboard.photo')} size="13|14" weight="semibold" color="gray-500" /></th>
              <th className="p-4"><Poppins text={t('dashboard.name')} size="13|14" weight="semibold" color="gray-500" /></th>
              <th className="p-4"><Poppins text={t('table.dniLicense')} size="13|14" weight="semibold" color="gray-500" /></th>
              <th className="p-4"><Poppins text={t('table.vehicle')} size="13|14" weight="semibold" color="gray-500" /></th>
              <th className="p-4"><Poppins text={t('dashboard.status')} size="13|14" weight="semibold" color="gray-500" /></th>
              <th className="p-4 text-right"><Poppins text={t('dashboard.actions')} size="13|14" weight="semibold" color="gray-500" /></th>
            </tr>
          </thead>
            ...
            {paginated.map((d) => (
              <tr key={d.id_conductor} className="hover:bg-gray-50/30 transition group">
                ...
                <td className="p-4">
                  {d.vehiculo ? (
                    <>
                      <Poppins text={`${d.vehiculo.marca} ${d.vehiculo.modelo}`} size="13|15" weight="medium" />
                      <div className="flex items-center gap-1.5 mt-0.5">
                        ...
                        <span className="w-2 h-2 rounded-full" style={{ backgroundColor: d.vehiculo.color === 'Blanco' ? '#f3f4f6' : d.vehiculo.color }}></span>
                      </div>
                    </>
                  ) : (
                ...
                <td className="p-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${
                    d.estado === 'disponible' ? 'bg-green-100 text-green-700' : 
                    d.estado === 'ocupado' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-700'
                  }`}>
                    {d.estado}
                  </span>
                </td>
      ...
      <div className="flex justify-between items-center mt-8 px-2">
          ...
          {[...Array(totalPages)].map((_, i) => (
            <button
              ...
              className={`w-10 h-10 rounded-xl transition font-bold text-sm ${
                page === i + 1 ? 'bg-primary text-white shadow-md shadow-primary/30' : 'hover:bg-gray-100 text-gray-500'
              }`}
            >
              {i + 1}
            </button>
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-xl transition font-medium ${
            page === totalPages ? 'opacity-30 cursor-not-allowed' : 'hover:bg-gray-100 text-gray-700'
          }`}
        >
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-gray-900/40 backdrop-blur-md p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in duration-300">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <div>
                <Poppins text={editingDriver ? t('dashboard.editDriver') : t('dashboard.newDriver')} size="20|24" weight="bold" />
                <Poppins text={editingDriver ? t('dashboard.editSubtitle') : t('dashboard.newSubtitle')} size="13|14" color="gray-500" />
              </div>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">{t('form.fullName')}</label>
                    <input
                      required
                      name="nombre"
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">{t('form.professionalEmail')}</label>
                    <input
                      required
                      type="email"
                      name="email"
                  ...
                  {!editingDriver && (
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">{t('form.password')}</label>
                      <input
                        required
                        type="password"
                        name="password"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">{t('form.dni')}</label>
                      <input
                        required
                        name="dni"
                        value={formData.dni}
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">{t('form.license')}</label>
                      <input
                        required
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">{t('form.plate')}</label>
                    <input
                      required
                      name="matricula"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">{t('form.brand')}</label>
                      <input
                        required
                        name="marca"
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">{t('form.model')}</label>
                      <input
                        required
                        name="modelo"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">{t('form.color')}</label>
                      <input
                        name="color"
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">{t('form.type')}</label>
                      <select
                        name="tipo"
                        value={formData.tipo}
                        onChange={handleInputChange}
              ...
              <div className="mt-10 flex gap-4">
                ...
                >
                  {isSubmitting ? t('dashboard.processing') : editingDriver ? t('dashboard.saveChanges') : t('dashboard.registerDriver')}
                </button>
              </div>
            </form>
          </div>
