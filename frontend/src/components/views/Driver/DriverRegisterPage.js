...
const InputField = ({
  ...
  return (
    <div className="space-y-1.5 w-full">
      ...
      {hint && !errors[id] && <p className="text-[11px] text-gray-400 ml-1 mt-1">{hint}</p>}
      {errors[id] && (
    ...
    if (step < 3) {
      ...
      if (res.error) {
        if (res.data?.errors) {
          ...
          if (
            res.data.errors.nombre ||
            res.data.errors.email ||
            res.data.errors.password ||
            res.data.errors.telefono ||
            res.data.errors.dni
          ) {
            setStep(1);
          ...
          <div className="space-y-4">
            {[
              ...
              >
                ...
                <div>
                  <Poppins
                    text={t(`benefits.items.${item.key}.title`)}
                    tag="h3"
                    size="16|18"
                    weight="semibold"
                  />
        ...
        <div className="w-full mx-auto rounded-3xl bg-white border border-gray-100 shadow-2xl shadow-gray-200/50 px-8 py-10">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="px-3 py-1 bg-primary/10 rounded-full">
              <Poppins
                text={t('form.stepCounter', { current: step, total: 3 })}
                size="12"
                weight="bold"
          ...
          <div className="flex items-center gap-3 mb-10">
            {[1, 2, 3].map((s) => (
              <div key={s} className="flex-1 flex items-center gap-2">
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold transition-all duration-300 ${step >= s ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'bg-gray-100 text-gray-400'}`}
                >
                  {s}
                </div>
                {s < 3 && (
                  <div
                    className={`flex-1 h-1 rounded-full ${step > s ? 'bg-primary' : 'bg-gray-100'}`}
                  />
                )}
              </div>
            ))}
          ...
          <div className="space-y-6">
            {step === 1 && (
              <div className="flex flex-col gap-5 animate-in fade-in slide-in-from-bottom-4">
                ...
                <InputField
                  label={t('form.fields.nombre.label')}
                  id="nombre"
            ...
            {step === 2 && (
              <div className="flex flex-col gap-5 animate-in fade-in slide-in-from-bottom-4">
                ...
                />
                <InputField
                  label={t('form.fields.numeroLicencia.label')}
                  id="numero_licencia"
                  placeholder="Ej: LZ-12345-A"
                  value={form.numero_licencia}
                  onChange={handleChange}
                  errors={errors}
                />
                <InputField
                  label={t('form.fields.licenciaExpira.label')}
                  id="licencia_expira"
                  type="date"
                  value={form.licencia_expira}
                  onChange={handleChange}
                  errors={errors}
                />
              </div>
            ...
            {step === 3 && (
              <div className="flex flex-col gap-5 animate-in fade-in slide-in-from-bottom-4">
                ...
                />
                <InputField
                  label={t('form.fields.matricula.label')}
                  id="matricula"
                ...
                <div className="grid grid-cols-2 gap-5">
                  <InputField
                    label={t('form.fields.marca.label')}
                    id="marca"
                  ...
                  />
                  <InputField
                    label={t('form.fields.modelo.label')}
                    id="modelo"
                ...
                <div className="grid grid-cols-3 gap-5">
                  <InputField
                    label={t('form.fields.plazas.label')}
                    id="plazas"
                    type="number"
                    placeholder="4"
                    value={form.plazas}
                  ...
                  />
                  <InputField
                    label={t('form.fields.anho.label')}
                    id="anio"
                    type="number"
                    placeholder="2024"
                    value={form.anio}
                  ...
                  <div className="space-y-1.5">
                    ...
                    />
                    <select
                      className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-primary/10 focus:border-primary transition-all"
                      value={form.tipo}
                ...
                <div className="space-y-3">
                  <Poppins
                    text={t('form.fields.color.label')}
                    tag="label"
                    size="14"
                    weight="medium"
                    className="text-gray-600 ml-1"
                  ...
                  <div className="flex flex-wrap gap-2">
                    {VEHICLE_COLORS.map((color) => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-xl border text-sm font-medium transition-all ${form.color === color ? 'bg-primary border-primary text-white shadow-md shadow-primary/20' : 'bg-white border-gray-100 text-gray-500 hover:border-primary/30'}`}
                      >
                        {color}
                      </button>
                    ))}
                ...
                <div
                  className={`flex items-center gap-3 p-4 rounded-2xl border transition-all ${form.aceptaTerminos ? 'bg-primary/5 border-primary/20' : 'bg-gray-50 border-gray-100'}`}
                >
                  <input
                    id="aceptaTerminos"
                    type="checkbox"
          ...
          <div className="flex gap-4 mt-10">
            {step > 1 && (
              <button
                onClick={handleBack}
                className="flex-1 px-6 py-4 bg-gray-50 text-gray-500 rounded-2xl font-bold hover:bg-gray-100 transition-all active:scale-[0.98]"
              >
                {t('form.buttons.back')}
              </button>
            )}
            ...
            >
              {submitting ? (
                ...
                <>
                  <Poppins
                    text={step === 3 ? t('form.buttons.submit') : t('form.buttons.next')}
                    tag="span"
                    color="white"
                    weight="bold"
                  />
                  <Icon name={step === 3 ? 'CheckCircle' : 'ArrowRight'} size={20} />
                </>
              )}
            </button>
          </div>
